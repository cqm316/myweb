layui.define(['element', 'common'], function (exports) {
    ...
    Navbar.prototype.on = function (events, callback) {
        ...
        var tabEven = function($btn){
            $btn.on('click', function () {
                var $a = $btn.children('a');
                var href = $a.data('url');
        ...
        if (eventName === 'click') {
            if (_con.attr('lay-filter') !== undefined) {
                _con.children('.layui-nav').find('.layui-nav-item').each(function () {
                    var $this = $(this);
    ...
    function getHtml(data) {
        //debugger;
        var ulHtml = '<div class="admin-side-toggle"><i class="fa fa-bars" aria-hidden="true"></i><em>系统导航</em></div>';
            ulHtml += '<div id="navItem" class="nav-item" style="display:none">系<br/>统<br/>导<br/>航</div>';
            ulHtml +='<ul class="layui-nav layui-nav-tree beg-navbar" id="begNarBar">';
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    var kItem = data[i].children[j];
                    if(data[i].children[j].spread){
                        ulHtml += '<dd class="layui-this" title="' + data[i].children[j].title + '">';
                    }else{
                        ulHtml += '<dd title="' + data[i].children[j].title + '">';
                    }
                    
                    if (kItem.children !== undefined && kItem.children !== null && kItem.children.length > 0) {
                        ulHtml += '<a class="a-nav" href="javascript:;" data-url="' + data[i].children[j].href + '"><i class="ico-s"></i>';
                    }else{
                    ...
                    if (kItem.children !== undefined && kItem.children !== null && kItem.children.length > 0) {
                        ulHtml += '<ul class="thirdNav">'
                        for (var k = 0; k < kItem.children.length; k++) {
                            var sItem = kItem.children[k];
                            ulHtml += '<li><a href="javascript:;" data-url="' + sItem.href + '"><i class="fa ' + sItem.icon + '"></i><cite>' + sItem.title + '</cite></a> </li>'
                        }
                        ulHtml += '</ul>'
                    }
